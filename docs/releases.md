...
GitHub prerelease assets are currently the only Runtime distribution channel. No Runtime artifacts are published to
Maven repositories, the Paper plugin portal, the Velocity plugin portal, or another package registry. Local Gradle
verification does not push tags, invoke the workflow, create attestations, or publish anything.
